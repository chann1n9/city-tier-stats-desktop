...
`docs/latest.json` 用于应用内检查最新版本。每次发布都需要根据本次版本同步更新，结构参考当前文件，不要改成 electron-builder 的 `latest.yml` 格式。
...
```json
{
  ...
  "downloads": {
    "mac_arm64": "https://downloads.eastun.tech/city-tier-stats-desktop/releases/v{version}/City.Tier.Stats-Mac-{version}.dmg",
    "win_x64": "https://downloads.eastun.tech/city-tier-stats-desktop/releases/v{version}/City.Tier.Stats-Windows-{version}-Setup.exe"
  },
  "releaseNotesUrl": "https://github.com/chann1n9/city-tier-stats-desktop/releases/tag/v{version}",
  "mandatory": false
}
```
...
- `version` 必须来自 `package.json` 的 `version`，不带 `v` 前缀
- `releaseDate` 使用生成发布文件当天的日期，格式为 `YYYY-MM-DD`
- `minimumVersion` 默认保留现有值；除非本次明确提高最低可更新版本，否则不要修改
- `notes` 写 1-5 条面向用户的简短更新摘要，优先从 Release Notes 的 `## 本次更新`、`## 改进`、`## 修复` 中提炼
- `downloads.mac_arm64` 和 `downloads.win_x64` 必须使用上面示例中的 OSS URL 前缀 `https://downloads.eastun.tech/city-tier-stats-desktop/releases/`
- 下载文件名必须和 Release Notes 下载区块中的产物命名保持一致
- `releaseNotesUrl` 指向对应版本的 GitHub Release 页面
- `mandatory` 默认写 `false`；只有本次明确要求用户必须升级时才写 `true`
- 不要在 `downloads` 中加入 `.blockmap`、调试文件、GitHub Release 链接或本地路径
- 不要把内部实现、构建流程、证书、公证、IPC、数据库等技术细节写入 `notes`
...
| 平台 | 下载 |
| --- | --- |
| Windows | [City.Tier.Stats-Windows-{version}-Setup.exe](https://github.com/chann1n9/city-tier-stats-desktop/releases/download/v{version}/City.Tier.Stats-Windows-{version}-Setup.exe) |
| macOS | [City.Tier.Stats-Mac-{version}.dmg](https://github.com/chann1n9/city-tier-stats-desktop/releases/download/v{version}/City.Tier.Stats-Mac-{version}.dmg) |
